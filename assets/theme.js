(() => {
  ...
  var OpenableElement = class extends CustomHTMLElement {
    ...
    constructor() {
      ...
      if (Shopify.designMode) {
        this.rootDelegate.on("shopify:section:select", (event) =>
          filterShopifyEvent(event, this, () => (this.open = true))
        );
        this.rootDelegate.on("shopify:section:deselect", (event) =>
          filterShopifyEvent(event, this, () => (this.open = false))
        );
      }
    ...
    async _load() {
      ...
      this.innerHTML = element.querySelector(
  ...
  window.customElements.define("loader-button", LoaderButton, {
    extends: "button",
  });
    ...
    set selectedIndex(selectedIndex) {
      ...
      if (this.hasAttribute("align-selected")) {
        const selectedItem = this.buttons[selectedIndex],
          ...
          scrollableElement = this._findFirstScrollableElement(
            this.parentElement
          );
        if (scrollableElement) {
          scrollableElement.scrollTo({
            ...
            left:
              ...
              (boundingRect.left - windowHalfWidth) +
    ...
    connectedCallback() {
      ...
      this.prevButton.addEventListener("click", () =>
        this.prevButton.dispatchEvent(
          new CustomEvent("prev-next:prev", { bubbles: true })
        )
      );
      this.nextButton.addEventListener("click", () =>
        this.nextButton.dispatchEvent(
          new CustomEvent("prev-next:next", { bubbles: true })
        )
      );
    }
  ...
  var PrevButton = class extends HTMLButtonElement {
    connectedCallback() {
      this.addEventListener("click", () =>
        this.dispatchEvent(new CustomEvent("prev-next:prev", { bubbles: true }))
      );
    }
  };
  var NextButton = class extends HTMLButtonElement {
    connectedCallback() {
      this.addEventListener("click", () =>
        this.dispatchEvent(new CustomEvent("prev-next:next", { bubbles: true }))
      );
    }
  };
  ...
  function getStickyHeaderOffset() {
    ...
    return (
      ...
      parseInt(
        documentStyles.getPropertyValue("--announcement-bar-height") || 0
      ) *
        parseInt(
          documentStyles.getPropertyValue("--enable-sticky-announcement-bar") ||
            0
        )
    );
  }
  ...
  var SafeSticky = class extends HTMLElement {
    ...
    get initialTopOffset() {
      return (
        getStickyHeaderOffset() + (parseInt(this.getAttribute("offset")) || 0)
      );
    }
    _checkPosition() {
  ...
  var ScrollSpy = class extends HTMLElement {
    connectedCallback() {
      ...
      window.addEventListener(
        "scroll",
        throttle(this.markVisibleSection.bind(this), 25)
      );
    ...
    syncPath() {
      const someElsAreVisible = () =>
          this.querySelectorAll(".is-visible").length > 0,
    ...
    markVisibleSection() {
      ...
      for (const [
        index,
        elementToObserve,
      ] of this.elementsToObserve.entries()) {
        const boundingClientRect = elementToObserve.getBoundingClientRect();
        if (
          boundingClientRect.top > getStickyHeaderOffset() ||
          index === this.elementsToObserve.length - 1
        ) {
          this.querySelector(
            `a[href="#${elementToObserve.id}"]`
...
`;
  ...
  var NativeCarousel = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      if (this.items.length > 1) {
        ...
        this.addEventListener("page-dots:changed", (event) =>
  ...
  var ScrollableContent = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.addEventListener(
        "scroll",
        throttle(this._calculateProgress.bind(this), 15)
      );
    }
    get draggable() {
  ...
  var SplitLines = class extends HTMLElement {
        ...
        requestAnimationFrame(() => {
          ...
          this.innerHTML = Object.values(bounds)
            .map(
              (item) =>
                `<span ${
                  this.hasAttribute("reveal") && !force ? "reveal" : ""
                } ${
                  this.hasAttribute("reveal-visibility") && !force
                    ? "reveal-visibility"
                    : ""
          ...
          this.style.visibility = this.hasAttribute("reveal-visibility")
            ? "visible"
            : null;
    ...
    async _onResize() {
      ...
      this.dispatchEvent(
        new CustomEvent("split-lines:re-split", { bubbles: true })
      );
      this.lastWidth = window.innerWidth;
    }
  };
  ...
  var CountrySelector = class extends HTMLSelectElement {
    connectedCallback() {
      ...
      const provinceSelectElement =
        this.provinceElement.tagName === "SELECT"
      ...
      if (provinceSelectElement.hasAttribute("data-default")) {
        for (let i = 0; i !== provinceSelectElement.options.length; ++i) {
          if (
            provinceSelectElement.options[i].text ===
            provinceSelectElement.getAttribute("data-default")
          ) {
  ...
  var FlickityControls = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      this.delegate.on("click", '[data-action="select"]', (event, target) =>
        this.flickityCarousel.select(`#${target.getAttribute("aria-controls")}`)
      );
    }
      ...
      let flickityInstance = await this.flickityCarousel.flickityInstance,
        activeItems = Array.from(
          this.querySelectorAll(
            `[aria-controls="${flickityInstance.selectedElement.id}"]`
          )
        );
      activeItems.forEach((activeItem) => {
        activeItem.setAttribute("aria-current", "true");
        getSiblings(activeItem).forEach((sibling) =>
    ...
    _setupPlayer() {
      ...
      return (this._setupPromise = new Promise((resolve) => {
        const template2 = this.querySelector("template"),
          node = template2.content.firstElementChild.cloneNode(true);
        node.onload = () => {
          this.hasLoaded = true;
          resolve();
        };
  ...
  var ModelMedia = class extends HTMLElement {
    ...
    async _setupShopifyXr() {
      if (!window.ShopifyXR) {
        ...
        const product = await ProductLoader.load(
          this.getAttribute("product-handle")
        );
        const models = product["media"].filter(
          (media) => media["media_type"] === "model"
        );
  ...
  var ComboBox = class extends OpenableElement {
    ...
    set selectedValue(value) {
      this.options.forEach((option) => {
        option.setAttribute(
          "aria-selected",
          option.getAttribute("value") === value ? "true" : "false"
        );
      });
    }
    attributeChangedCallback(name, oldValue, newValue) {
      ...
      switch (name) {
        case "open":
          if (this.open) {
            ...
            this.classList.toggle(
              "combo-box--top",
              boundingRect.top >= (window.innerHeight / 2) * 1.5
            );
            setTimeout(() => this.focusTrap.activate(), 150);
          } else {
            this.focusTrap.deactivate();
  ...
  var AnnouncementBar = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      if (Shopify.designMode) {
        this.delegate.on("shopify:block:select", (event) =>
  ...
  var AnnouncementBarItem = class extends CustomHTMLElement {
    ...
    get focusTrap() {
      return (this._trapFocus =
        this._trapFocus ||
        createFocusTrap(
          this.contentElement.querySelector(".announcement-bar__content-inner"),
          {
            fallbackFocus: this,
            clickOutsideDeactivates: (event) =>
    ...
    async openContent() {
      if (this.hasContent) {
        ...
        this.dispatchEvent(
          new CustomEvent("announcement-bar:content:open", { bubbles: true })
        );
      }
    }
    async closeContent() {
      ...
      await new Promise((resolve) => {
        ...
        this.dispatchEvent(
          new CustomEvent("announcement-bar:content:close", { bubbles: true })
        );
      });
    }
  };
  ...
  var ProductRecommendations = class extends HTMLElement {
    async connectedCallback() {
      ...
      const response = await fetch(
        `${window.themeVariables.routes.productRecommendationsUrl}?product_id=${this.productId}&limit=${this.recommendationsCount}&section_id=${this.sectionId}`
      );
      const div = document.createElement("div");
      div.innerHTML = await response.text();
  ...
  function getMediaSrcset(media, sizeList) {
    let srcset = [],
      supportedSizes =
        typeof media === "string"
          ? sizeList
          : getSupportedSizes(media, sizeList);
    supportedSizes.forEach((supportedSize) => {
      srcset.push(
        `${getSizedMediaUrl(media, supportedSize + "x")} ${supportedSize}w`
      );
    });
    return srcset.join(",");
  }
  ...
  var GroupEffect = class {
    ...
    get animationEffects() {
      return this._childrenEffects.flatMap((effect) => {
        return effect instanceof CustomKeyframeEffect
          ? effect
  ...
  var Slideshow = class extends CustomHTMLElement {
    ...
    get transitionType() {
      return MediaFeatures.prefersReducedMotion()
        ? "fade"
    ...
    async select(index, shouldTransition = true, reverseDirection = false) {
      ...
      if (previousItem && previousItem !== newItem) {
        if (this.transitionType !== "reveal") {
          ...
          await previousItem.transitionToLeave(
            this.transitionType,
            shouldTransition
          );
        }
  ...
  var ShopTheLookItem = class extends HTMLElement {
    ...
    async transitionToEnter(shouldAnimate = true) {
      ...
      const animation = new CustomAnimation(
        new SequenceEffect([
          new ParallelEffect(
            Array.from(this.querySelectorAll(".shop-the-look__image")).map(
              (item) => {
                return new CustomKeyframeEffect(
                  item,
                  { opacity: [1, 1] },
                ...
                Array.from(
                  ...
                  return new CustomKeyframeEffect(
                    textItem,
                    {
                      ...
                      clipPath: [
                        `inset(${
                          prefersReducedMotion ? "0 0 0 0" : "0 0 100% 0"
                        })`,
                        "inset(0 0 0 0)",
                      ],
                      transform: [
  ...
  var ProductList = class extends CustomHTMLElement {
    ...
    previous(event) {
      const directionFlip =
          window.themeVariables.settings.direction === "ltr" ? 1 : -1,
        columnGap = parseInt(
          getComputedStyle(this).getPropertyValue("--product-list-column-gap")
        );
      ...
      console.log(this.productListInner);
      ...
      switch (name) {
        case "hidden":
          ...
          if (!this.hidden) {
            ...
            (_a = this.querySelector(".prev-next-button--prev")) == null
              ? void 0
              : _a.setAttribute("disabled", "");
            (_b = this.querySelector(".prev-next-button--next")) == null
              ? void 0
    ...
    async _staggerProductsApparition() {
      ...
      await this.untilVisible({
        threshold:
          this.clientHeight > 0 ? Math.min(50 / this.clientHeight, 1) : 0,
      });
          ...
          this.items.map((item, index) => {
            return new CustomKeyframeEffect(
              item,
              {
                ...
                transform: [
                  `translateY(${
                    MediaFeatures.prefersReducedMotion() ? 0 : "30px"
                  })`,
                  "translateY(0)",
                ],
              },
  ...
  var BlogPostNavigation = class extends HTMLElement {
    connectedCallback() {
      window.addEventListener(
        "scroll",
        throttle(this._updateProgressBar.bind(this), 15)
      );
    }
    get hasNextArticle() {
    ...
    _updateProgressBar() {
      ...
      this.classList.toggle(
        ...
        articleMainPartBoundingBox.top < stickyHeaderOffset &&
          articleMainPartBoundingBox.bottom >
            stickyHeaderOffset + this.clientHeight - marginCompensation
      );
  ...
  var MultiColumn = class extends CustomHTMLElement {
    ...
    previous(event) {
      const directionFlip =
          window.themeVariables.settings.direction === "ltr" ? 1 : -1,
        columnGap = parseInt(
          getComputedStyle(this).getPropertyValue("--multi-column-column-gap")
        );
      event.target.nextElementSibling.removeAttribute("disabled");
      event.target.toggleAttribute(
        "disabled",
  ...
  var ImageWithTextOverlay = class extends CustomHTMLElement {
    ...
    async _setupVisibility() {
      ...
      const animation = prefersReducedMotion
        ? new CustomAnimation(new ParallelEffect(innerEffect))
        : new CustomAnimation(new SequenceEffect(innerEffect));
      this._hasSectionReloaded ? animation.finish() : animation.play();
    }
    _onScroll() {
      ...
      requestAnimationFrame(() => {
        ...
        if (contentElement) {
          contentElement.style.opacity = Math.max(
            1 -
              speedFactor *
                (1 - Math.min(boundingRectBottom / boundingRectHeight, 1)),
            0
          ).toString();
        }
        if (imageElement) {
            ...
            Math.max(
              1 -
                (1 -
                  Math.min(
                    boundingRectBottom /
                      (boundingRectHeight + stickyHeaderOffset),
                    1
                  )),
              0
            ) *
  ...
  var PredictiveSearchDrawer = class extends DrawerContent {
    ...
    async _doPredictiveSearch(term) {
      const response = await fetch(
        ...
        }?q=${encodeURIComponent(term)}&resources[limit]=10&resources[type]=${
    ...
    async _doLiquidSearch(term) {
      ...
      supportedTypes.forEach((searchType) => {
        promises.push(
          fetch(
            `${window.themeVariables.routes.searchUrl}?section_id=predictive-search-compatibility&q=${term}&type=${searchType}&options[unavailable_products]=${window.themeVariables.settings.searchUnavailableProducts}&options[prefix]=last`,
            {
              signal: this.abortController.signal,
            }
          )
  ...
  var Timeline = class extends HTMLElement {
    ...
    _onMediaChanged(event) {
      if (event.matches) {
        ...
        this.listItemElements.forEach((item) =>
          this.itemIntersectionObserver.unobserve(item)
        );
      }
    }
  };
  ...
  var DesktopNavigation = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.delegate.on("shopify:block:deselect", (event) =>
    ...
    openDropdown(parentElement) {
      ...
      this.currentMegaMenu = dropdown.classList.contains("mega-menu")
        ? dropdown
        : null;
      let openingTimeout = setTimeout(() => {
        ...
        openingTimeout = null;
        this.dispatchEvent(
          new CustomEvent("desktop-nav:dropdown:open", { bubbles: true })
        );
      }, 100);
    ...
    closeDropdown(parentElement) {
      ...
      requestAnimationFrame(() => {
        ...
        setTimeout(
          ...
          dropdown.classList.contains("mega-menu") &&
            this.currentMegaMenu !== dropdown
            ? 250
            : 0
        );
        this.dispatchEvent(
          new CustomEvent("desktop-nav:dropdown:close", { bubbles: true })
        );
      });
    }
  };
  ...
  var MobileNavigation = class extends DrawerContent {
    get apparitionAnimation() {
      ...
      if (!MediaFeatures.prefersReducedMotion()) {
        ...
        return (this._apparitionAnimation = new CustomAnimation(
          new ParallelEffect(effects)
        ));
      }
    }
    attributeChangedCallback(name, oldValue, newValue) {
    ...
    connectedCallback() {
      ...
      if (this.isTransparent) {
        ...
        if (this.isSticky) {
          ...
          this._onWindowScrollListener = throttle(
            this._checkTransparentHeader.bind(this),
            100
          );
  ...
  var PhotoSwipeUi = class {
    ...
    init() {
      const prevNextButtons = this.pswpUi.querySelector(
      ...
      dotsNavWrapper.querySelector(
        ".pswp__dots-nav-wrapper .dots-nav"
      ).innerHTML = dotsNavHtml;
    }
    _close() {
    ...
    _onPswpDoubleTap(point) {
      ...
      if (this.photoSwipeInstance.getZoomLevel() !== initialZoomLevel) {
        ...
        this.photoSwipeInstance.zoomTo(
          initialZoomLevel < 0.7 ? 1 : this.maxSpreadZoom,
          point,
          333
        );
      }
    ...
    _onPswpBeforeChange() {
      ...
      getSiblings(activeDot).forEach((item) =>
      ...
      return Promise.resolve(
        activeImages.map((item) => {
          const matchedMedia = product["media"].find(
              (media) =>
                media.id === parseInt(item.getAttribute("data-media-id"))
            ),
  ...
  var PaymentContainer = class extends HTMLElement {
    ...
    _updateAddToCartButton(variant) {
      ...
      if (addToCartButtonElement.getAttribute("is") === "loader-button") {
        addToCartButtonElement.firstElementChild.innerHTML =
          addToCartButtonText;
      } else {
  ...
  var ProductMedia = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      this.mainCarousel.addEventListener("model:played", () =>
        this.mainCarousel.setDraggable(false)
      );
      this.mainCarousel.addEventListener("model:paused", () =>
        this.mainCarousel.setDraggable(true)
      );
    ...
    async _setupVisibility() {
      ...
      const flickityInstance = await this.mainCarousel.flickityInstance,
        image = flickityInstance
          ? flickityInstance.selectedElement.querySelector("img")
    ...
    async _onVariantChanged(event) {
      ...
      if (
        currentlyFilteredIds.some((value) => !filteredMediaIds.includes(value))
      ) {
        const selectedMediaId = variant["featured_media"]
          ? variant["featured_media"]["id"]
          : this.product["media"]
              .map((item) => item.id)
              .filter((item) => !filteredMediaIds.includes(item))[0];
        Array.from(this.querySelectorAll("[data-media-id]")).forEach((item) => {
          item.classList.toggle(
            "is-filtered",
            filteredMediaIds.includes(
  ...
  var ProductMeta = class extends HTMLElement {
    ...
    _updatePrices(variant) {
      let productPrices = this.querySelector("[data-product-price-list]"),
        currencyFormat = window.themeVariables.settings.currencyCodeEnabled
    ...
    _updateSku(variant) {
      ...
      let productSkuNumber = productSku.querySelector(
  ...
  var QuickBuyDrawer = class extends DrawerContent {
    ...
    _onVariantChanged(event) {
      ...
      this.imageElement.setAttribute(
        "src",
        getSizedMediaUrl(featuredMedia, "342x")
      );
      this.imageElement.setAttribute(
        "srcset",
        getMediaSrcset(featuredMedia, [114, 228, 342])
      );
    }
  };
  ...
  var QuickBuyPopover = class extends PopoverContent {
    ...
    _onVariantChanged(event) {
      ...
      this.imageElement.setAttribute(
        "src",
        getSizedMediaUrl(featuredMedia, "195x")
      );
      this.imageElement.setAttribute(
        "srcset",
        getMediaSrcset(featuredMedia, [65, 130, 195])
      );
    }
  };
      ...
      const applyClassToSelector = (
        ...
        switch (selectorType) {
          ...
          case "block":
            ...
            break;
          case "dropdown":
            cssSelector = `.combo-box__option-item:nth-child(${
              valueIndex + 1
            })`;
            break;
        }
    ...
    async _rerender(event) {
      ...
      try {
        ...
        const activeFilterList = Array.from(
            fakeDiv.querySelectorAll(".product-facet__active-list")
          ),
        ...
        if (filtersTempDiv) {
          ...
          Array.from(
            ...
            const filtersTempDivToggle = filtersTempDiv.querySelector(
                `[aria-controls="${filterToggle.getAttribute(
                  "aria-controls"
                )}"]`
              ),
              isExpanded =
          ...
          this.querySelector("#facet-filters .drawer__content").scrollTop =
            previousScrollTop;
        }
        const scrollTo =
  ...
  var FacetFilters = class extends DrawerContent {
    ...
    _onFilterChanged() {
      ...
      if (window.innerWidth < 1000) {
        const productType = searchParams.getAll("filter.p.product_type");
        if (productType) {
          const firstItem = productType[0];
          searchParams.set("filter.p.product_type", firstItem);
        }
  ...
  var _CartDrawerRecommendations = class extends HTMLElement {
    async connectedCallback() {
      if (!_CartDrawerRecommendations.recommendationsCache[this.productId]) {
        _CartDrawerRecommendations.recommendationsCache[this.productId] = fetch(
          `${window.themeVariables.routes.productRecommendationsUrl}?product_id=${this.productId}&limit=10&section_id=${this.sectionId}`
        );
      }
      const response = await _CartDrawerRecommendations.recommendationsCache[
        this.productId
      ];
      const div = document.createElement("div");
      ...
      if (
        ...
        productRecommendationsElement.hasChildNodes()
      ) {
        this.innerHTML = productRecommendationsElement.innerHTML;
      } else {
        this.hidden = true;
  ...
  var FreeShippingBar = class extends HTMLElement {
    ...
    _onCartUpdated(event) {
      this.style.setProperty(
        "--progress",
        Math.min(
          parseFloat(event.detail["cart"]["total_price"]) / this.threshold,
          1
        )
      );
    }
  ...
  var ShippingEstimator = class extends HTMLElement {
    ...
    async _estimateShipping() {
      const zip = this.querySelector('[name="shipping-estimator[zip]"]').value,
        country = this.querySelector(
          '[name="shipping-estimator[country]"]'
        ).value,
        province = this.querySelector(
    ...
    _formatShippingRates(shippingRates) {
      var _a;
      (_a = this.querySelector(".shipping-estimator__results")) == null
        ? void 0
        : _a.remove();
      let formattedShippingRates = "";
      shippingRates.forEach((shippingRate) => {
        formattedShippingRates += `<li>${
          shippingRate["presentment_name"]
        }: ${formatMoney(parseFloat(shippingRate["price"]) * 100)}</li>`;
      });
      const html = `
    ...
    _onClick() {
      ...
      if (window.matchMedia(window.themeVariables.breakpoints.pocket).matches) {
        ...
        document
          .querySelector(
            `[aria-controls="${
              shopifyReviewsElement.closest(".product-tabs__tab-item-wrapper")
                .id
            }"]`
          )
          .click();
  ...
  var ProductStickyForm = class extends HTMLElement {
    ...
    _onVariantChanged(event) {
      ...
      if (this.priceElement) {
        this.priceElement.innerHTML = formatMoney(
          variant["price"],
          currencyFormat
        );
      }
      if (this.unitPriceElement) {
        this.unitPriceElement.style.display = variant["unit_price_measurement"]
          ? "block"
          : "none";
        if (variant["unit_price_measurement"]) {
          let referenceValue = "";
      ...
      this.imageElement.setAttribute(
        "src",
        getSizedMediaUrl(featuredMedia, "165x")
      );
      this.imageElement.setAttribute(
        "srcset",
        getMediaSrcset(featuredMedia, [55, 110, 165])
      );
    }
          ...
          entries.forEach((entry) => {
            ...
            if (entry.target === paymentContainerElement) {
              ...
              this._isPaymentContainerPassed =
                ...
                boundingRect.top + boundingRect.height <= stickyHeaderOffset;
            }
          });
          if (
            ...
            this.hidden =
              !this._isPaymentContainerPassed || this._isFooterVisible;
          } else {
